function testNeuralNetStochastic()
    M = 2;
    %initialW1 = ones(2, M);
%     initialW1 = [1, 2; 3, 4];
    %initialW2 = ones(M, 3);
%     initialW2 = [1, 2, 3; 4, 5, 6];
     %hiddenNodes = [1 2 5 10 25 50 100 150 200];
     hiddenNodes = [25];
...
%         initialW2 = [1, 2, 3; 4, 5, 6];
        i
        [w1, w2, error] = stochasticGradientDescentNoLoops(Xtrain, Ytrain, 0.1*initialW1, 0.1*initialW2, 0.01, 0.001, 0.1, hiddenNodes(i),K);
%         [w1, w2, error] = gradientDescentNoLoops(Xtrain, Ytrain, initialW1, initialW2, 0.01, 0.001*hiddenNodes(i)*K, 0, hiddenNodes(i),K);
        ...
        errorT = getClassificationError(Xtrain, Ytrain, max(Ytrain), w1, w2, hiddenNodes(i));
        error = getClassificationError(Xvalidate, Yvalidate, max(Yvalidate), w1, w2, hiddenNodes(i));
        errorTraining(1, i) = errorT;
     ...
     X = data(:,1:784);
     Y = data(:,785);
     error = getClassificationError(X, Y, max(Y), w1, w2, hiddenNodes(i))
    errorMatrix
    hiddenNodes
    scatter(hiddenNodes, errorTraining);
    hold on
    scatter(hiddenNodes, errorMatrix);
